 ...
 * @return {String}
 */
export function fattenPhone(phone) {
	return `${phone.slice(0, 3)}****${phone.slice(7, 11)}`
}

/**
 * 格式化图表数据（name含有名称  数量  百分比）
 * @return {Array}
 */
export function fattenChartData(arr) {
	return arr.typeList.map(item => {
		return {
			name: `${item.name.slice(0, 10)}   ${Number(item.num)}   ${calcValueRate(arr.typeList, Number(item.num))}`,
...
function calcValueRate(arr, val) {
	const arrNum = arr.map(item => {
		return Number(item.num)
	})
	return (parseFloat(val / arrNum.reduce((sum, a) => {
		return sum += a
	})) * 100).toFixed(2) + '%'
}
